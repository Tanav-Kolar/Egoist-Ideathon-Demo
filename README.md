...
Built for the Egoist AI Passport Ideathon. See [`build-spec.md`](build-spec.md) for the full product spec and [`CLAUDE.md`](CLAUDE.md) for the build's steering rules (visual skin, persona, model constraints).
...
- **Platform connectors** (Shutterstock, Getty, Behance, …) are an illustrative integration model, not live production integrations.
...
The registry never stores a policy — only a pointer: `fingerprint → policyRef`. Resolving a check is always a live two-step lookup (registry → vault), so editing the standing policy (narrowing the use scope, turning off AI training, letting it expire) changes the answer for every already-registered fingerprint on its very next check. Open devtools → Application → Local Storage → `coathook_v1` and inspect a registry entry yourself — it will never contain a policy, an image, or a name.
...
assets/
  css/coathook.css          Egoist visual skin (tokens, components)
